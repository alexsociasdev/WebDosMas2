    ...
    <section className="rounded-3xl border border-base-mid bg-white p-8 shadow-soft">
      ...
      <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {logos.map((logo) => (
          <article
            key={logo.name}
            className="group flex flex-col items-center justify-center rounded-2xl border border-base-mid bg-base-light p-4 transition hover:-translate-y-1 hover:border-brand-yellow"
          >
            <div className="relative h-16 w-full">
              <Image src={logo.src} alt={logo.name} fill className="object-contain" loading="lazy" />
            </div>
            <p className="mt-3 text-center text-sm font-medium text-base-dark">{logo.name}</p>
          </article>
        ))}
      </div>
    </section>
